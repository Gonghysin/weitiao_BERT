...
from sklearn.metrics import accuracy_score; from sklearn.metrics import f1_score  # 评估指标
...
def evaluate_model(model, data_loader, device):
    ...
    with torch.no_grad():  # 禁用梯度计算
        for batch in data_loader:  # 遍历每个batch
            ...
            # 修改此行，添加device_type参数
            with torch.amp.autocast(device_type=device.type):  # 指定设备类型
                outputs = model(input_ids=input_ids, attention_mask=attention_mask)
...
def main():
    ...
    # 加载数据
    logger.info("加载数据集...")
    train_data = load_data("/root/weitiao_BERT--/IMCS-DAC_train.json")
    val_data = load_data("/root/weitiao_BERT--/IMCS-DAC_dev.json")
    test_data = load_data("/root/weitiao_BERT--/IMCS-DAC_test.json")
    ...
    # 新增目录创建代码
    os.makedirs("models", exist_ok=True)  # 自动创建models目录
